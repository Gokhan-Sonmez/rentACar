...
public class ColorManager implements ColorService {
	...
	public DataResult<List<ColorListDto>> getAll() {
		...
		return new SuccessDataResult<List<ColorListDto>>(response, Messages.colorListed);
	}

	@Override
	public DataResult<ColorDto> getByColorId(int id) {

		Color color = this.colorDao.getById(id);
		ColorDto response = this.modelMapperService.forDto().map(color, ColorDto.class);

		return new SuccessDataResult<ColorDto>(response, Messages.colorListed);
	}
